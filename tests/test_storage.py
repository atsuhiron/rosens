...
def test_save_creates_new_file(tmp_path: Path) -> None:
    ...
    storage.save(ENVIRONMENT, _environment_data("sensor-1"), RECIEVED_AT)
...
def test_save_appends_to_existing_file(tmp_path: Path) -> None:
    ...
    storage.save(ENVIRONMENT, _environment_data("sensor-1"), RECIEVED_AT)
    storage.save(ENVIRONMENT, _environment_data("sensor-2"), RECIEVED_AT)
...
def test_load_spans_multiple_days(tmp_path: Path) -> None:
    ...
    for sensor_id, recieved_at in times.items():
        storage.save(ENVIRONMENT, _environment_data(sensor_id), recieved_at)
...
def test_load_includes_boundary_timestamps(tmp_path: Path) -> None:
    ...
    rows = storage.load(ENVIRONMENT, RECIEVED_AT, after)
...
def test_load_returns_empty_when_no_files(tmp_path: Path) -> None:
    ...
    rows = storage.load(ENVIRONMENT, RECIEVED_AT, RECIEVED_AT + timedelta(days=1))
